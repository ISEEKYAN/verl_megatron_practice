...
-   **Qwen3.5-35B-A3B Megatron and Megatron-FSDP 128K benchmark**
    -   **Note**: Megatron-only harness, no veRL rollout/inference throughput included.
    -   **Results**: [qwen3_5_megatron.csv](./qwen3_5_megatron.csv)
    -   **Megatron-FSDP rows**: Their `pp*ep` values use `1*EP` because Megatron-FSDP does not support pipeline parallelism.
